...
var resFromGit = function(getObj, destPath, response, callback) {
  ...
  git.clone(uri, getObj.ref, outDir, response, function(err) {
    if (err) {
      callback(err);
    } else {
...
/*
 * FIXME: the whole directory must be scanned only one time.
 */
exports.renameForWpkg = function(root) {
  const uniq = {};
  const rename = [];
  const list = xFs.lsall(root);
  list.forEach(entry => {
  ...
  rename.forEach((entry, idx) => {
    xFs.mv(entry, `${entry}__peon_${idx}__`);
  });

  xFs.batch.mv(/^(ChangeLog)$/i, '$1__peon__', root);
  xFs.batch.mv(/:/g, '__peon_colon__', root);
};
  ...
  async.auto(
    {
      ...
      configure: [
        ...
        function(callback, results) {
          if (
            (!extra.forceConfigure && !results.mount) ||
            !extra.configure ||
            !extra.configure.length
          ) {
            callback();
          ...
          interpreter.run(extra.configure, response, err => {
            if (results.mount) {
...
const rpathFixupDarwin = watt(function*(prefix, response, next) {
  ...
  for (const dylib of _dylibs) {
    ...
    if (!isSymlink) {
      response.log.info(`set rpath id for ${dylibFull} to @rpath/${dylib}`);
      try {
  ...
  try {
    bins = bins.concat(
      xFs
        ...
        .filter(bin => !fs.lstatSync(bin).isSymbolicLink())
    );
  } catch (ex) {
    if (ex.code !== 'ENOENT') {
      throw ex;
  ...
  for (const bin of bins) {
    try {
      yield* installNameTool(
        ['-rpath', rootLibDir, '@executable_path/../lib', bin],
        next
      );
    ...
    for (const dep of otoolRes) {
      response.log.info(`change rpath of ${dep} for ${bin}`);
      yield* installNameTool(
...
const rpathFixupLinux = watt(function*(prefix, response, next) {
  ...
  for (const file of files) {
    response.log.info(`try to fix rpath for ${file}`);
    try {
      yield* patchelf(
        ['--set-rpath', `$ORIGIN/../lib:${rootLibDir}`, file],
        next
      );
    } catch (ex) {}
  }
...
exports.rpathFixup = watt(function*(prefix, response) {
  switch (xPlatform.getOs()) {
    case 'darwin': {
      return yield rpathFixupDarwin(prefix, response);
    }

    case 'linux': {
      return yield rpathFixupLinux(prefix, response);
    }
  }
});

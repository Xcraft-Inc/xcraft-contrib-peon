...
resFrom.cache = watt(function* (protocol, getObj, destPath, resp, next) {
  ...
  if (getObj.$hash) {
    const cachedVersion = getObj.$hash.split('@', 1)[0];
    const cachedFile = path.join(cacheRoot, getObj.$hash);
    if (cachedVersion === getObj.$version && fse.existsSync(cachedFile)) {
  ...
  for (let attempt = 1; attempt <= 10; ++attempt) {
    try {
      ...
      const fileHash = (yield fileChecksum(res, {algorithm: 'sha256'})) + ext;
      const hash = `${getObj.$version}@${fileHash}`;
...
exports.prepare = watt(function* (
  ...
  const globalDir = extra.name.split(/[+-]/);
  const assetsGlobalDir = path.join(
    xConfig.pkgProductsRoot,
    `.${globalDir[0]}`,
    'assets',
    globalDir[1]
  );
  ...
  xPh.global
    ...
    .set('ASSETS.GLOBAL', normalize(assetsGlobalDir))
    .set('POSIX.ASSETS.GLOBAL', normalize(assetsGlobalDir, true))
...
const rpathFixupDarwin = watt(function* (params, resp, next) {
  ...
  /**
   * Get existing RPATHs from a binary
   * @yields
   * @param {string} bin Binary path
   * @param {Function} next Callback
   * @returns {string[]} List of existing RPATHs
   */
  function* getRPATHs(bin, next) {
    let res = [];
    try {
      yield* otool(['-l', bin], (line) => res.push(line), next);
    } catch (ex) {
      return [];
    }

    const rpaths = [];
    let isRpathSec = false;
    ...
    for (let line of res) {
      ...
      line = line.trim();
      if (isRpathSec && line.startsWith('path')) {
  ...
  /**
   * Remove duplicate RPATHs from a binary
   * @yields
   * @param {string} bin Binary path
   * @param {Function} next Callback
   */
  function* removeDuplicateRPATHs(bin, next) {
    const rpaths = yield* getRPATHs(bin, next);

    let counter = {};
    for (const rpath of rpaths) {
      counter[rpath] = (counter[rpath] || 0) + 1;
    }

    counter = Object.entries(counter).filter(([, count]) => count > 1);
    for (const [rpath, count] of counter) {
      log(resp).info(
        `Removing ${count - 1} duplicate(s) of ${rpath} from ${bin}`
      );
      for (let i = 1; i < count; ++i) {
        try {
          yield* installNameTool(['-delete_rpath', rpath, bin], next);
        } catch (ex) {
          log(resp).warn(
            `Failed to delete duplicate RPATH ${rpath}: ${ex.message}`
          );
          break;
        }
      }
   ...
   *    Look for path with otool -L (maybe it's related to a symbolic link)
   *    Use install_name_tool -change
   * 6. Remove duplicated RPATHs if needed
   */
  ...
  for (const bin of bins) {
    ...
    try {
      yield* removeDuplicateRPATHs(bin, next);
    } catch (ex) {
      log(resp).warn(
        `Failed to clean duplicate RPATHs for ${bin}: ${ex.message}`
      );
    }
  }
});

...
var resFromGit = function(getObj, destPath, response, callback) {
  ...
  git.clone(options, response, function(err) {
    if (err) {
      callback(err);
    } else {
  ...
  xFs.batch.mv((location, file) => {
    ...
    if (st && st.isSymbolicLink()) {
      ...
      if ((stl && stl.isDirectory()) || !fullTarget.startsWith(root)) {
        ...
        fs.writeFileSync(fullPath, target, {mode: st.mode});
...
const rpathFixupDarwin = watt(function*(params, response, next) {
  let {prefix} = params;
  ...
  for (const dylib of _dylibs) {
    ...
    if (!isSymlink) {
      log(response).info(`set rpath id for ${dylibFull} to @rpath/${dylib}`);
      try {
  ...
  for (const bin of bins) {
    ...
    for (const dep of otoolRes) {
      log(response).info(`change rpath of ${dep} for ${bin}`);
      yield* installNameTool(
...
const rpathFixupLinux = watt(function*(params, response, next) {
  ...
  for (const file of files) {
    log(response).info(`try to fix rpath for ${file}`);
    try {
      const relativePath = path.relative(
        path.dirname(file),
        path.join(prefix, 'lib')
      ...
      yield* patchelf(
        ['--set-rpath', `$ORIGIN/${relativePath}:${rootLibDir}`, file],
        next
      );
    } catch (ex) {
      /* ignore errors */
...
exports.rpathFixupDir = watt(function*(prefix, libDir, binDir, response) {
  const params = {prefix, libDir, binDir};
  switch (xPlatform.getOs()) {
    case 'darwin': {
      return yield rpathFixupDarwin(params, response);
    }

    case 'linux': {
      return yield rpathFixupLinux(params, response);
    }
  }
});

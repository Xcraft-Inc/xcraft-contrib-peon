...
exports.renameForWpkg = function(root) {
  ...
  xFs.batch.mv((location, file) => {
    ...
    newFileName = fileRegex(file, /^(ChangeLog)$/i, '$1__peon__');
    if (!newFileName) {
      newFileName = fileRegex(file, /:/g, '__peon_colon__');
    }

    const norm = path.join(location, (newFileName || file).toLowerCase());
    if (uniq.hasOwnProperty(norm)) {
      newFileName = `${newFileName || file}__peon_${uniq[norm]}__`;
      ++uniq[norm];
    } else {
      uniq[norm] = 0;
    }

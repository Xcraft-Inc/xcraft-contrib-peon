...
resFrom.cache = function (protocol, getObj, destPath, resp, callback) {
  ...
  if (fse.existsSync(cachedFile)) {
    resp.log.info(`retrieve "${path.basename(file)}" from the cache`);
  ...
  handler(getObj, destPath, outputFile, resp, (err, res) => {
    if (!err) {
      resp.log.info(`store ${path.basename(file)} in the cache`);
  ...
  async.auto(
    {
      ...
      configure: [
        ...
        function (callback, results) {
          ...
          interpreter.run(extra.configure, null, resp, (err) => {
            if (results.fetch.location) {
              process.chdir(currentDir);
            }

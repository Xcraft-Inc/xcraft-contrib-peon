...
var resFromGit = function (gitUri, destPath, callback) {
  ...
  git.clone (gitUri, outDir, function (err) {
    if (err) {
      callback (err);
    } else {
...
exports.prepare = function (uri, root, share, extra, callback) {
  var async    = require ('async');
  var traverse = require ('traverse');
  var xConfig  = require ('xcraft-core-etc').load ('xcraft');

  /* Set default placeholder */
  xPh.global.set ('XCRAFTHOME', path.join (xConfig.xcraftRoot, '/home'));
  xPh.global.set ('WPKGSTAG', root);

...
const rpathFixupLinux = watt(function* (params, resp, next) {
  ...
  for (const file of files) {
    ...
    try {
      if (
        fse.existsSync(path.join(path.dirname(file), '../lib/libc.so.6')) ||
        fse.existsSync(path.join(path.dirname(file), '../lib64/libc.so.6'))
      ) {
        /* We must always relies to the host libc */
        yield* patchelf(
          ['--set-rpath', `${rootLibDir}:${rootLibDir64}`, file],
          next
        );
      } else {
        const relativePath = path.relative(

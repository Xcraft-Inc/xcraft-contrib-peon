...
exports.prepare = function (getObj, root, share, extra, callback) {
  ...
  async.auto ({
    ...
    mount: ['fetch', function (callback, results) {
      /* 0. No mount with unices because the MAX_PATH problem doesn't exists.
       * 1. Nothing to mount if the target doesn't exists.
       * 2. Nothing to configure.
       */
      if (/* (0) */ xPlatform.getOs () !== 'win' ||
          /* (1) */ !fs.existsSync (results.fetch) ||
          /* (2) */ !extra.configure || !extra.configure.length) {
        callback (null, results.fetch);
        return;
      }

      console.log ('mount ' + results.fetch);
      subst = new xPlatform.Subst (results.fetch);

...
const rpathFixupDarwin = watt (function * (prefix, response, next) {
  ...
  for (const bin of bins) {
    try {
      yield * installNameTool (['-rpath', rootLibDir, '@executable_path/../lib', bin], next);
...
const rpathFixupLinux = watt (function * (prefix, response, next) {
  /* chrpath is optional, skip this step until that the chrpath package
   * is installed.
   */
  try {
    which.sync ('chrpath');
  ...
  function * chrpath (args, next) {
    return yield xProcess.spawn ('chrpath', args, {}, next);
  }
  ...
  for (const file of files) {
    ...
    try {
      yield * chrpath (['-r', `$ORIGIN/../lib:${rootLibDir}`, file], next);
    } catch (ex) {}
  }
});

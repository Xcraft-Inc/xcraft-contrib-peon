...
resFrom.cache = watt(function* (protocol, getObj, destPath, resp, next) {
  ...
  for (let attempt = 1; attempt <= 3; ++attempt) {
    try {
      ...
      if (attempt < 3) {
        resp.log.warn(
          `error with ${basename}, attempt (${attempt + 1}) in 3s: ${ex}`
        );
...
function generateDebugEnv(share, extra) {
  const debugEnv = path.join(share, 'source-debug-env.sh');
  ...
  fse.writeFileSync(debugEnv, script);
}
...
exports.prepare = function (
  ...
  xPh.global
    .set(
      'XCRAFT.HOME',
      path.join(xConfig.xcraftRoot, '/home').replace(/\\/g, '/')
    )
    .set('WPKG.STAG', basePath.replace(/\\/g, '/'))
    ...
    .set('ROOTDIR', rootDir.replace(/\\/g, '/'))
    .set('PROD.ROOTDIR', targetRoot.replace(/\\/g, '/'))
    .set('ENV', process.env)
    .set('ROOTTEMP', xConfig.tempRoot.replace(/\\/g, '/'))
    .set(
      'ROOTDRIVE',
  ...
  if (from === 'onlyBuild') {
     ...
     * It's then possible to use the source-debug-env.sh script in order
  ...
  async.auto(
    {
      ...
      prepare: [
        ...
        (callback, results) => {
          ...
          interpreter.run(extra.prepare, null, resp, (err) => {
      ...
      configure: [
        ...
        function (callback, results) {
          ...
          interpreter.run(extra.configure, env, resp, (err) => {
            if (results.fetch.location) {
              process.chdir(currentDir);
            }
    ...
    function (err, results) {
      ...
      if (subst) {
        if (
          !subst.skipped &&
          results.fetch &&
          results.fetch.location.slice(0, 2) !== subst.location.slice(0, 2)
        ) {
          /* Restore original location */
          results.fetch.location = path.resolve(
            path.normalize(subst.location),
            path
              .normalize(results.fetch.location)
              .split(path.sep)
              .splice(1)
              .join(path.sep)
          );

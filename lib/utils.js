...
var resFromHttp = function(getObj, destPath, resp, callback) {
...
const resFromFtp = function(getObj, destPath, resp, callback) {
...
var resFromGit = function(getObj, destPath, resp, callback) {
  ...
  git.clone(options, resp, function(err) {
    if (err) {
      callback(err);
    } else {
  ...
  switch (uriObj.protocol) {
    ...
    case 'https:': {
      if (/\.git$/.test(uriObj.pathname)) {
        resFromGit(getObj, destPath, resp, function(err, res) {
          if (err) {
            callback(err);
            return;
          }
      ...
      resFromHttp(getObj, destPath, resp, function(err, res) {
        if (err) {
          callback(err);
          return;
        }
    ...
    case 'ftp:': {
      resFromFtp(getObj, destPath, resp, (err, res) => {
        if (err) {
          callback(err);
          return;
        }
...
exports.prepare = function(getObj, root, share, extra, resp, callback) {
  var os = require('os');
  ...
  async.auto(
    {
      fetch: function(callback) {
        ...
        if (!fs.existsSync(cache)) {
          fileFromUri(getObj, share, resp, callback);
        } else {
          callback(null, cache);
        }
      },
      ...
      mount: [
        ...
        function(callback, results) {
          ...
          resp.log.verb('mount ' + results.fetch);

          subst = new Subst(results.fetch, resp);
      ...
      umount: [
        ...
        function(callback, results) {
          ...
          resp.log.verb('umount ' + results.mount);
